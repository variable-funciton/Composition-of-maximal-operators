...
st.title("Compositions of maximal operators behaviour:\n $\\left( M_{\\alpha}\\circ M_{\\beta}\\right) \\left[ \\chi_{[0,1]}\\right](x)$, $M_{\\alpha+\\beta} \\left[ \\chi_{[0,1]} \\right](x)$, $\\left( M_{\\beta}\\circ M_{\\alpha} \\right)\\left[ \\chi_{[0,1]}\\right](x)$ and $M^{2}\\left[ \\chi_{[0,1]}\\right](x)$")
...
with st.sidebar:
    ...
    show_frac_maximal = st.checkbox("Show M_{{α+β}}", value=False)
    show_composition_reverse = st.checkbox("Show M_{{β}}◦M_{{α}}", value=False)
    # ビューレンジの計算
...
def fractional_maximal(x,A):
    ...
    idx_pos = x > 1
    xx_p = x[idx_pos]
    val[idx_pos] = xx_p**(A-1)
    idx_neg = x < 0
    xx_n = 1 - x[idx_neg]
    val[idx_neg] = xx_n**(A-1)
    return val
...
ax.plot(x_plot, y_ab, lw=2.5, color="royalblue", label=rf"$M_{{\alpha}}\circ M_{{\beta}} = M_{{{alpha:.3f}}} \circ M_{{{beta:.3f}}}$")

if show_m2:
    ax.plot(x_plot, MM_iterated_correct(x_plot), lw=1.5, color="#333333", alpha=0.7, label=rf"$M\circ M \cong M_{{L\log L}}$")
if show_frac_maximal:
    ax.plot(x_plot, fractional_maximal(x_plot, S), lw=1.5, color="green", alpha=0.7, label = rf"$M_{{\alpha + \beta}} = M_{{{S}}}$")
if show_composition_reverse:
    ax.plot(x_plot, y_ba, lw=2.5, color="crimson", ls="--", label=rf"$M_{{\beta}}\circ M_{{\alpha}} = M_{{{beta:.3f}}} \circ M_{{{alpha:.3f}}}$")
...
st.latex(r"M_{\alpha+\beta} \left[ \chi_{[0,1]}\right](x) = \begin{cases} 1 & (0 \le x \le 1) \\ x^{\alpha+\beta-1} & (x > 1) \\ (1-x)^{\alpha+\beta-1} & (x < 0) \end{cases}")
st.write("and")
...
st.write("---")
st.write(r"The $x$-value that gives a local maximum for $\beta>0$:")
...
st.write(r"The $x$-value that gives a local maximum for $\beta = 0$:")
st.write(r"$x_0 = e^{\frac{\alpha}{1-\alpha}}$ and $x_1 = 1-e^{\frac{\alpha}{1-\alpha}}$.")
